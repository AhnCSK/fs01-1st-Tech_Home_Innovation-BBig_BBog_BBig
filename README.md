...
| 문제                       | 원인                         | 해결 방안                             |
| ------------------------ | -------------------------- | --------------------------------- |
| **문제1: MQTT 통신 불안정**     | MQTT토픽 설정 미흡        | MQTT토픽 체계화하여 관리       |
| **문제2: 스마트 화분 물 펌프 고장** | 펌프 하드웨어 고장 | 펌프 하드웨어 교체 및 연결 확인           |
| **문제3: 비밀번호 해시 미적용**     | 로그인 기능 개발 시 암호화 고려 부족      | SHA-256 또는 BCrypt 적용, DB 저장 방식 개선 |
| **문제4: 관리자 계정 관리 불가**    | 관리자 계정 추가/삭제 기능 미구현        | 관리자 계정 CRUD 기능 구현                 |
| **문제5: 마트 물품 구입 기능 미구현** | 관련 테이블 미사용      | 마트 테이블 생성, 구매/결제 기능 구현            |
...
| 기능/문제     | 결과              | 발생 문제/원인                    | 해결 방안                             | 개선 필요점                   | 우선순위  |
| --------- | --------------- | --------------------------- | --------------------------------- | ------------------------ | ----- |
| MQTT 통신   | 성공              | MQTT 토픽 설정 미흡, 센서 제어 기능 오작동 | MQTT 토픽 체계화, 통신 로그 분석             | 통신 안정화 및 고도화             | 🟠 중간 |
| 전등/창문 제어  | 성공              | 센서 데이터 처리 및 명령 전달 지연        | 센서 데이터 처리 코드 최적화          | 센서 데이터 처리 및 명령 전달 딜레이 감소 | 🟡 낮음 |
| 침입자 감지    | 성공              | 센서 데이터 처리 지연                | 센서 데이터 처리 코드 최적화          | 센서 데이터 처리 및 명령 전달 딜레이 감소    | 🟡 낮음 |
| 화재 감지     | 성공              | 센서 데이터 처리 지연                | 센서 데이터 처리 코드 최적화          | 센서 데이터 처리 및 명령 전달 딜레이 감소    | 🟡 낮음 |
| 스마트 화분 제어 | 실패              | 펌프 하드웨어 고장                  | 펌프 하드웨어 교체 및 연결 확인                | 펌프 제어 기능 복구              | 🔴 높음 |
| 로그인/회원가입  | 성공              | 비밀번호 해시 미적용                 | SHA-256 또는 BCrypt 적용, DB 저장 방식 개선 | 비밀번호 해시화                 | 🔴 높음 |
| 관리자 기능    | 부분 성공 (글 관리 가능) | 관리자 계정 추가/삭제 기능 미구현         | 관리자 계정 추가/삭제 기능 구현                 | 관리자 계정 추가/삭제 기능          | 🟠 중간 |
| 마트 물품 구입  | 실패              | 관련 테이블 및 결제/구매 로직 미설계       | 마트 테이블 생성, 구매/결제 기능 구현            | 마트 테이블 추가 및 구매/결제 기능 구현  | 🟠 중간 |
