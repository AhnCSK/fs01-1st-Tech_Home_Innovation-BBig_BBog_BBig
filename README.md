...
| 기능             | 구현 결과             | 이슈 추적                  | 처리방안                            | 개선 필요점           | 우선순위 |
| :------------- | :---------------- | :--------------------- | :------------------------------- | :--------------- | :--: |
| **MQTT 통신**    | 구현 성공             | 토픽 체계 미흡, 일부 센서 제어 오작동 | MQTT 토픽 명명 규칙 정립 및 통신 로그 분석      | 통신 안정화 및 고도화     | 🟠 중 |
| **Threads 통신** | 구현 성공             | Thread 연결 불안정          | Thread 재사용 방지 및 실행 시 새 Thread 생성 | Thread 재사용 구조 개선 | 🟠 중 |
| **전등/창문 제어**   | 구현 성공             | 센서 데이터 처리 및 명령 전달 지연   | 데이터 처리 로직 최적화                    | 명령 전달 속도 개선      | 🟡 하 |
| **침입자 감지 팝업**  | 구현 성공             | 센서 데이터 처리 지연           | 데이터 처리 알고리즘 최적화                  | 반응 속도 개선         | 🟡 하 |
| **화재 감지 팝업**   | 구현 성공             | 센서 데이터 처리 지연           | 데이터 처리 코드 개선                     | 반응 속도 개선         | 🟡 하 |
| **스마트 화분 제어**  | 부분 성공 (하드웨어 고장)   | 펌프 고장                  | 하드웨어 교체 및 연결 확인                  | 펌프 제어 기능 복구      | 🔴 상 |
| **로그인/회원가입**   | 구현 성공             | 비밀번호 해시 미적용            | SHA-256 또는 BCrypt 적용             | 비밀번호 암호화 강화      | 🔴 상 |
| **관리자 기능**     | 부분 성공 (게시글 관리 가능) | 계정 추가/삭제 불가            | 관리자 전용 CRUD 기능 추가                | 계정 관리 기능 개선      | 🟠 중 |
| **마트 물품 구입**   | 미구현               | 테이블 및 결제 모듈 미비         | DB 테이블 생성 및 결제 기능 구현             | 구매/결제 기능 구현      | 🟠 중 |

| 기능             | 구현 결과             | 이슈 추적                  | 개선 필요점           | 처리방안                             | 우선순위 |
| :------------- | :---------------- | :--------------------- | :--------------- | :------------------------------- | :--: |
| **스마트 화분 제어**  | 부분 성공 (하드웨어 고장)   | 펌프 고장                  | 펌프 제어 기능 복구      | 하드웨어 교체 및 연결 확인                  | 🔴 상 |
| **로그인/회원가입**   | 구현 성공             | 비밀번호 해시 미적용            | 비밀번호 암호화 강화      | SHA-256 또는 BCrypt 적용             | 🔴 상 |
| **MQTT 통신**    | 구현 성공             | 토픽 체계 미흡, 일부 센서 제어 오작동 | 통신 안정화 및 고도화     | MQTT 토픽 명명 규칙 정립 및 통신 로그 분석      | 🟠 중 |
| **Threads 통신** | 구현 성공             | Thread 연결 불안정          | Thread 재사용 구조 개선 | Thread 재사용 방지 및 실행 시 새 Thread 생성 | 🟠 중 |
| **관리자 기능**     | 부분 성공 (게시글 관리 가능) | 계정 추가/삭제 불가            | 계정 관리 기능 개선      | 관리자 전용 CRUD 기능 추가                | 🟠 중 |
| **마트 물품 구입**   | 미구현               | 테이블 및 결제 모듈 미비         | 구매/결제 기능 구현      | DB 테이블 생성 및 결제 기능 구현             | 🟠 중 |
| **전등/창문 제어**   | 구현 성공             | 센서 데이터 처리 및 명령 전달 지연   | 명령 전달 속도 개선      | 데이터 처리 로직 최적화                    | 🟡 하 |
| **침입자 감지 팝업**  | 구현 성공             | 센서 데이터 처리 지연           | 반응 속도 개선         | 데이터 처리 알고리즘 최적화                  | 🟡 하 |
| **화재 감지 팝업**   | 구현 성공             | 센서 데이터 처리 지연           | 반응 속도 개선         | 데이터 처리 코드 개선                     | 🟡 하 |
