...
| 기능     | 결과              | 문제                    | 해결 방안                             | 개선 필요점                   | 우선순위  |
| --------- | --------------- | --------------------------- | --------------------------------- | ------------------------ | ----- |
| MQTT 통신   | 구현성공              | MQTT 토픽 설정 미흡, 센서 제어 기능 오작동 | MQTT 토픽 체계화, 통신 로그 분석             | 통신 안정화 및 고도화             | 🟠 중 |
| 전등/창문 제어  | 구현성공              | 센서 데이터 처리 및 명령 전달 지연        | 센서 데이터 처리 코드 최적화          | 센서 데이터 처리 및 명령 전달 딜레이 감소 | 🟡 하 |
| 침입자 감지 팝업   | 구현성공              | 센서 데이터 처리 지연                | 센서 데이터 처리 코드 최적화          | 센서 데이터 처리 및 명령 전달 딜레이 감소    | 🟡 하 |
| 화재 감지 팝업     | 구현성공              | 센서 데이터 처리 지연                | 센서 데이터 처리 코드 최적화          | 센서 데이터 처리 및 명령 전달 딜레이 감소    | 🟡 하 |
| 스마트 화분 제어 | 부분 성공 (하드웨어 고장)              | 펌프 하드웨어 고장                  | 펌프 하드웨어 교체 및 연결 확인                | 펌프 제어 기능 복구              | 🔴 상 |
| 로그인/회원가입  | 성공              | 비밀번호 해시 미적용                 | SHA-256 또는 BCrypt 적용, DB 저장 방식 개선 | 비밀번호 해시화                 | 🔴 상 |
| 관리자 기능    | 부분 성공 (글 관리 가능) | 관리자 계정 추가/삭제 기능         | 관리자 계정을 admin아이디로 구현                 | 관리자 계정 추가/삭제 기능 개선          | 🟠 중 |
| 마트 물품 구입  | 실패              | 관련 테이블 및 결제/구매       | 마트 테이블 생성, 구매/결제 기능 구현            | 마트 테이블 추가 및 구매/결제 기능 구현  | 🟠 중 |

| 기능             | 구현 결과             | 주요 문제                  | 해결 방안                            | 개선 필요점           | 우선순위 |
| :------------- | :---------------- | :--------------------- | :------------------------------- | :--------------- | :--: |
