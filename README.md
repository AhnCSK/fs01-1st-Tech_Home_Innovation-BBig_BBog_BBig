...
| 기능             | 구현 결과             | 이슈 추적                  | 처리결과                            | 개선 필요점           | 우선순위 |
| :------------- | :---------------- | :--------------------- | :------------------------------- | :--------------- | :--: |
...
| 기능             | 주요 이슈                  | 우선순위 | 개선 필요점           | 처리결과                             | 구현 상태 |
| :------------- | :--------------------- | :--: | :--------------- | :------------------------------- | :---- |
| **MQTT 통신**    | 토픽 체계 미흡, 일부 센서 제어 오작동 | 🟠 중 | 통신 안정화 및 고도화     | MQTT 토픽 명명 규칙 정립 및 통신 로그 분석      | 구현 성공 |
| **Threads 통신** | Thread 연결 불안정          | 🟠 중 | Thread 재사용 구조 개선 | Thread 재사용 방지 및 실행 시 새 Thread 생성 | 구현 성공 |
| **전등/창문 제어**   | 데이터 처리 및 명령 전달 지연      | 🟡 하 | 명령 전달 속도 개선      | 데이터 처리 로직 최적화                    | 구현 성공 |
| **침입자 감지 팝업**  | 센서 데이터 처리 지연           | 🟡 하 | 반응 속도 개선         | 데이터 처리 알고리즘 최적화                  | 구현 성공 |
| **화재 감지 팝업**   | 센서 데이터 처리 지연           | 🟡 하 | 반응 속도 개선         | 데이터 처리 코드 개선                     | 구현 성공 |
| **스마트 화분 제어**  | 하드웨어(펌프) 고장            | 🔴 상 | 펌프 제어 기능 복구      | 하드웨어 교체 및 연결 확인                  | 부분 성공 |
| **로그인/회원가입**   | 비밀번호 해시 미적용            | 🔴 상 | 비밀번호 암호화 강화      | SHA-256 또는 BCrypt 적용             | 구현 성공 |
| **관리자 기능**     | 계정 추가/삭제 불가            | 🟠 중 | 계정 관리 기능 개선      | 관리자 전용 CRUD 기능 추가                | 부분 성공 |
| **마트 물품 구입**   | 테이블 및 결제 모듈 미비         | 🟠 중 | 구매/결제 기능 구현      | DB 테이블 생성 및 결제 기능 구현             | 미구현   |
